...
4. **Phase 2 Stage 2c** ([#550](https://github.com/rent-a-vacation/rav-website/issues/550)) — Interactive editing + per-user scenarios on `/executive-dashboard/financial-model`. Sparse-override Supabase table; drift banner; reset-to-baseline; live commission read from DB (closes small drift surface today). ~4-6 days. **Next-session pickup** after current pipeline clears. Then **Stage 2d** ([#551](https://github.com/rent-a-vacation/rav-website/issues/551)) — functional .xlsx download from web (refactor build.ts → pure function; two download buttons baseline/scenario; ~3-5 days). Together 2c + 2d let the web replace the .xlsx as primary tool. **Stage 2b ([#545](https://github.com/rent-a-vacation/rav-website/issues/545)) resequenced post-launch** — see DEC-042 revision (live actuals overlay has nothing to overlay pre-launch).
...
## Current Priority Tiers (as of May 15, 2026 — Session 68 Part 4 close)
...
| **#545** | Phase 2 Stage 2b — Live actuals overlay on /executive-dashboard/financial-model | **Resequenced from Tier A to post-launch (Session 68 close, 2026-05-16).** Live actuals require launch data to validate — Model Month 5+ minimum. Most valuable Month 12+ when forecast-vs-actual variance becomes the conversation. Stages 2c (#550) + 2d (#551) take priority pre-launch. See DEC-042 revision in PROJECT-HUB for full rationale. |
...
|------|---------|---------|
| May 16, 2026 | 68 (post-close) | **Phase 2 Stage resequence + scope tickets filed.** Per founder discussion: live actuals overlay (Stage 2b / #545) resequenced from Tier-A next-session-pickup to post-launch positioning. Stages 2c (interactive editing + scenarios — **#550**) and 2d (functional .xlsx download — **#551**) take precedence pre-launch because (a) live actuals has nothing to overlay until launch Month 5+; (b) 2c removes daily friction NOW (founders open .xlsx weekly to tweak); (c) 2c+2d together let web replace .xlsx as primary tool. DEC-042 updated in PROJECT-HUB with full design lock: two-category baseline (live-config Category A read from DB per DEC-043; forecast-assumption Category B sparse-overrides in Supabase), per-user scenarios with optional share-to-team flag, localStorage drafts, persistent drift banner + per-input dots, CLI as absolute truth with two web download buttons (baseline + active scenario). docs/financials/README.md gains upcoming-stages section. **#545 body updated** with resequence note + label flipped pre-launch → post-launch. **No code changes.** Sub-Session 68 close PR. |
| May 16, 2026 | 68 (Part 4 close) | **Documentation management framework — all 4 PRs landed (PR #540 + #541 + #543 + this PR).** DEC-044 logged in PROJECT-HUB Key Decisions Log. CLAUDE.md gains a new "Documentation Management Framework" section (top-priority no-duplication rule, `/sdlc-docs` + `/generate-docs` skill summary, naming + archive conventions, source-doc-map ownership, SECURITY-RISK-LOG triggers). "What NOT to Do" extended with 2 new rules: never duplicate doc content; never edit snapshots in `docs/exports/` directly. `/sdlc` skill session-end checklist gains a SECURITY-RISK-LOG triage checkpoint. Session 68 Part 4 handoff finalized in PROJECT-HUB. **6 follow-up issues filed** across the framework: #535 (functional-search-bar README), #536 (mdm-resort-data README), #537 (posthog-analytics README), #538 (travel-request-enhancements README), #539 (voice-auth-approval README), #542 (docs-audit.ts should respect .gitignore). **Weekly `npm audit` `/schedule` routine recommended** but deferred to user opt-in (uses API quota). **Auto-archive `archive-stale --apply` deferred** as a follow-up (file moves are hard to reverse; manual review preferred for v1). **No code changes; tests 1778 unchanged; migrations 080 unchanged.** Framework now self-enforcing — next PR-to-main will run through both `docs-audit` + `sdlc-docs audit --gate`. |
| May 15, 2026 | 68 (Part 4) | **Documentation management framework — PR1 (accounting + INDEX + features cleanup).** Top-priority no-doc-duplication rule locked into auto-memory. PRICING-TAXES-ACCOUNTING bumped to v2.3 (Phase H status corrected from "DEV complete pending PR to main" to "✅ Complete in PROD per PRs #272/#294"; #531/#532 follow-ups noted). New `docs/INDEX.md` (master doc map), `docs/features/accounting/README.md` (thin index, no duplication), `docs/financials/README.md` (clarifies gitignored .xlsx + canonical TS source). 3 missing-feature READMEs written (notification-center, resort-master-data, seo-optimization); 5 follow-up issues filed (#535-#539). PLATFORM-REVIEW-03022026.md (actually a UX review) renamed + archived to `docs/archive/Platform-UX-Review-2026-03-02.md` with stub redirect; new naming convention enforced in INDEX.md. Executive Dashboard README drift fixed (claimed "Not built" but 15 components + 5 hooks exist). DEC-044 scaffolded for next-session ship: `/sdlc-docs` (doc-sync watchdog) + `/generate-docs` (snapshot composer) + SECURITY-RISK-LOG sync (PRs 2-4 deferred). **No code changes; tests 1778 unchanged; migrations 080 unchanged.** |
